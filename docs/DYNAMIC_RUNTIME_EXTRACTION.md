...
或脚本。动态模式可由本地 CLI 直接运行，也可由默认关闭、仅 loopback 的本地 Web UI
用固定参数代理；两者都要求用户显式信任会被执行的发布 JAR。Web UI 不是公网部署目标。
...
用户后续在真实客户端中确认该 DP 能加载并出现新内容，但炮塔缺少弹药，退出地图时
崩溃。该 Mod 大量依赖自定义 Java 内容与行为，不适合作为兼容率样本；这项结果说明
运行时观测与 clean apply 仍无法自动迁移任意方法语义。
